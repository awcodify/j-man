...
var (
	roundDBTypes = map[string]string{`ID`: `integer`, `Name`: `text`, `Description`: `text`, `Status`: `character varying`, `Users`: `bigint`, `RampUp`: `bigint`, `Duration`: `bigint`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`, `DeletedAt`: `timestamp with time zone`}
	_            = bytes.MinRead
)
